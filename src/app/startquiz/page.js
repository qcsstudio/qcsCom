...
const Page = () => {
  ...
  useEffect(() => {
    // localStorage browser me hi chalega
  ...
  const fetchStudents = async (id) => {
    try {
      if (id) {
        const res = await fetch(`http://localhost:3000/api/candidate?id=${id}`)
        if (!res.ok) {
  ...
  console.log(students, "studentsstudentsstudentsstudentsstudents")
  return (
    <>
      <div className="p-6">
