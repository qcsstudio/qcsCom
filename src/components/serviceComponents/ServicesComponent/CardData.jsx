 export const CardData = [
    {
      services:["MERN Stack",
      "MEAN Stack",
      "RESTful APIs & GraphQL Integration",
      "Cross-Platform Web App Development",
      "E-Commerce & CMS Solutions",
      "Authentication, Security & Performance",
      "Deployment & CI/CD Workflows",],
      heading:" Custom Web Development",
      desc:"Build fast, scalable, and secure applications with QuantumCrafters Studio. From sleek websites to complex web apps, we deliver seamless frontend and backend solutions using modern stacks.",
      link:""
    },
    {
        services:["User Research & Analysis",
          "Wireframing & Prototyping",
          "Interactive UI Design",
          "UX Strategy & Optimization",
          "Mobile & Web App Design",
          "Design System & Brand Consistency",
          "Usability Testing & Enhancements"
        ],
        heading : " UI/UX Design & Experience",
        desc:"Create interfaces that users love to explore and interact with. We blend design thinking with usability to craft impactful digital experiences that are both functional and beautiful.",
        link:""
      },
    {
      services:["SEO & On-Page Optimization",
        "Performance Marketing (Google, Meta, LinkedIn)",
        "Email Campaign Automation",
        "AI-Powered Content & Hashtag Research",
        "LinkedIn Lead Gen & Personal Branding",
        "Marketing Funnels & A/B Testing",
        "CRM Integration & Analytics"
      ],
        heading:" Digital Marketing & Automation",
        desc:"Grow faster with ROI-driven strategies and AI-powered campaigns. We blend organic reach and paid ads with automation to generate consistent, high-quality leads and brand visibility.",
        
    },
   
    
    {
      services:["AI Opportunity Mapping",
        "Proof of Concept (PoC) Creation",
        "Process Automation Roadmapping",
        "Integration with Existing Tools",
        "Cost-Benefit & ROI Forecasting",
        "Scalable ML Solution Planning",
        "AI Infrastructure Setup"
      ],
      heading:" AI Strategy & Consulting",
      desc:"Empower your business with intelligent automation. We help you plan, build, and deploy scalable AI systems tailored to your workflows and goals.",
      
    },
    {
      services:["Process Workflow Automation",
        "Task Automation via Python, Zapier & Make.com",
        "HR, Sales & CRM Integrations",
        "Document & Email Automation",
        "Custom AI Bots & Scripts",
        "Dashboard + Notification Systems",
        "Low-Code/No-Code Apps"
      ],
     heading:" Business Process Automation",
     desc:"Automate the mundane and scale the impactful. Whether it’s internal ops or customer journeys, we deploy intelligent workflows to simplify execution."
    },
    {
      services:["Data Collection & Cleanup",
        "Trend Analysis & Forecasting",
        "Customer Behavior Prediction",
        "Real-Time Reporting Dashboards",
        "Visualization (Power BI, Tableau)",
        "Recommendation Systems",
        "Demand Planning Models"
      ],
     heading:"  Predictive Analytics & Insights",
     desc:"Turn data into smart decisions. We help you uncover trends, predict outcomes, and optimize strategies using ML & AI-powered forecasting."
    },
    {
      services:["AWS, Azure, GCP Cloud Setup",
        "CI/CD Pipelines & GitOps",
        "Docker, Kubernetes, & Containers",
        "Cloud Monitoring & Auto-Scaling",
        "Infrastructure as Code (IaC)",
        "Deployment Automation",
        "Performance Optimization"
      ],
     heading:"  DevOps & Cloud Engineering",
     desc:"Deploy better, scale smarter. We provide agile, secure, and scalable DevOps & Cloud solutions for product startups and growing businesses."
    },
    {
      services:["AI Integration with CRMs, ERPs & CMSs",
        "Chatbots, NLP & Virtual Assistants",
        "Recommendation Engines & Smart Search",
        "AI-Powered Automation for Workflows",
        "Third-Party API & SaaS AI Plug-ins",
        "Real-Time AI Data Processing",
        "Voice, Vision & Generative AI Connectors"
      ],
     heading:" End-to-End AI Integrations",
     desc:"Seamless Intelligence Across Your Business Stack.We help businesses embed AI directly into their tools, workflows, and platforms—turning everyday systems into smart, self-learning assets."
    },
    
    ];

export const LinkData = {
  links:[
    "/eduservice",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
  ]
}
 